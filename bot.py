...
async def get_ton_price_toman() -> float:
    """دریافت قیمت TON به تومان از Nobitex"""
    try:
        async with aiohttp.ClientSession() as session:
            url = "https://api.nobitex.ir/v2/orderbook/TONIRT"
            headers = {"User-Agent": "Mozilla/5.0"}
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=8)) as resp:
                data = await resp.json()
                # بهترین قیمت خرید از orderbook
                bids = data.get("bids", [])
                if bids:
                    price_irt = float(bids[0][0])  # قیمت به ریال
                    return price_irt / 10  # تبدیل به تومان
        return 0
    except Exception as e:
        logger.error(f"get_ton_price_toman error: {e}")
        return 0

async def toman_to_ton(toman: int) -> float:
    """تبدیل مبلغ تومان به TON"""
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "check_join":
        if await is_member(context.bot, uid):
            ...
            await context.bot.send_message(uid, "به ربات خوش آمدید! 👋", reply_markup=main_kb(uid))
        else:
...
async def do_ton_payment(q, uid, key, context):
    ...
    if ton_amount <= 0:
        await q.edit_message_text(
            "⚠️ در حال حاضر امکان دریافت قیمت TON وجود ندارد. لطفاً با روش دیگری پرداخت کنید.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ بستن", callback_data="cancel")]])
        )
        return
