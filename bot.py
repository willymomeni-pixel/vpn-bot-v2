...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    state = gs(uid)
    w = state.get("w")
    if w == "receipt":       await recv_receipt(update, context)
    elif w == "topup_receipt": await recv_topup_receipt(update, context)
...
async def recv_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.photo:
        ...
        await msg.reply_text("لطفاً تصویر رسید را ارسال کنید.")
        return
